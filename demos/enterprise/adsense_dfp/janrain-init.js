...
/** Functions to handle DFP **************/
    ...
    janrain.events.onCaptureLoginSuccess.addHandler(function(result){
        ...
        // after the user authenticates, pull additional demographics from the
        // "userData" object, and then invoke the rest of the DFP code to show
        // the banner ad, including demographics for custom targeting:
        displayDFPAd(result.userData);
    ...
    janrain.events.onCaptureRegistrationSuccess.addHandler(function(result){
        ...
        // first we check to see if this was a traditional registration (using email/password instead of
        // a social provider). If so, let's not show the banner ad yet, because the user is going to get
        // a specific part of the flow where they are told that they should go verify their email address,
        // and after that screen closes, our widget's session is automatically terminated.
        // So:

        if (result.action != "traditionalRegister") {
            // Same call as above in the onCaptureLoginSuccess event:
            displayDFPAd(result.userData);
            document.getElementById("captureSignInLink").style.display='none';
            document.getElementById("captureSignOutLink").style.display='';
            document.getElementById("captureProfileLink").style.display='';
        ...
        // Note: your own design and requirements will of course help determine when you should show banner
        // ads; this is just a reference implementation to help illustrate some of the aspects of integrating
        // DFP with Janrain Enterprise.
